...
注意：你只需要标记哪些是无效章节，不需要删除任何内容。程序会保留所有章节，只是将无效章节标记出来供用户查看和手动处理。
...
例如：前一章是第25章，中间出现"第九场获胜了的第九场"，后面是第26章，这个"第九场获胜了的第九场"是正文内容被误判为章节，应标记为无效。
...
- is_valid为false表示该章节被标记为无效，用户之后可以手动删除
