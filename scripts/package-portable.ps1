...
$Exe = Join-Path $ReleaseDir "novel-processor.exe"
$PortableExeName = "NovelProcessor.exe"
$OutDir = Join-Path $Root "portable"
$Version = (Get-Content -Encoding UTF8 (Join-Path $Root "package.json") | ConvertFrom-Json).version
$PackageDir = Join-Path $OutDir "NovelProcessor-v$Version-windows-x64"
$StagingDir = Join-Path $OutDir ".staging-$([guid]::NewGuid().ToString('N'))"
$ZipPath = Join-Path $OutDir "NovelProcessor-v$Version-windows-x64.zip"
...
Get-ChildItem -LiteralPath $OutDir -Force | Where-Object {
  $_.Name -like "NovelProcessor-v*-windows-x64" -or $_.Name -like "NovelProcessor-v*-windows-x64.zip"
} | Where-Object {
  $_.Name -ne $PackageName -and $_.Name -ne $ZipName
} | Remove-Item -Recurse -Force

New-Item -ItemType Directory -Path $StagingDir | Out-Null
Copy-Item -LiteralPath $Exe -Destination (Join-Path $StagingDir $PortableExeName)
