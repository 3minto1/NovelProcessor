...
export type SearchSide = "original" | "corrected";
...
export function buildSearchMatches(
  ...
  for (const chapter of chapters) {
    ...
    if (scope !== "corrected") sides.push(["original", chapter.original_text]);
    if (scope !== "original" && chapter.corrected_text?.trim()) {
      sides.push(["corrected", chapter.corrected_text]);
    }
