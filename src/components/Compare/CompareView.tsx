...
type CompareViewProps = {
  ...
  correctedRef: RefObject<HTMLDivElement>;
      ...
      {open && (
        ...
        >
          {chapters.map((chapter, index) => {
            const completed = chapter.review_status === "completed"
              && Boolean(chapter.corrected_text?.trim());
            return (
              <button
                type="button"
                key={chapter.id}
...
function emptyDiffState(chapterId: string, original: string, corrected: string, loading = false): DiffState {
  return { ranges: [], mode: "mixed", loading, chapterId, original, corrected, cached: false };
}

export function useChapterDiff(chapterId: string, original: string, corrected: string, enabled: boolean): DiffState {
  const requestIdRef = useRef(0);
  const [state, setState] = useState<DiffState>(() => emptyDiffState(chapterId, original, corrected));

  useEffect(() => {
    const requestId = requestIdRef.current + 1;
    requestIdRef.current = requestId;
    if (!enabled || !corrected.trim()) {
      setState(emptyDiffState(chapterId, original, corrected));
      return undefined;
    }

    const cached = getCachedDiff(chapterId, original, corrected);
    if (cached) {
      setState({ ...cached, loading: false, chapterId, original, corrected, cached: true });
      return undefined;
    }

    let cancelled = false;
    setState(emptyDiffState(chapterId, original, corrected, true));
    if (typeof Worker === "undefined") {
      Promise.resolve().then(() => calculateDiff(original, corrected)).then((result) => {
        if (!cancelled && requestIdRef.current === requestId) {
          setCachedDiff(chapterId, original, corrected, result);
          setState({ ...result, loading: false, chapterId, original, corrected, cached: false });
        }
      }).catch((error) => {
        if (!cancelled && requestIdRef.current === requestId) {
          setState({ ...emptyDiffState(chapterId, original, corrected), error: String(error) });
        }
      });
    ...
    worker.onmessage = (event: MessageEvent<{ requestId: number; result?: DiffResult; error?: string }>) => {
      ...
      if (event.data.result) {
        setCachedDiff(chapterId, original, corrected, event.data.result);
        setState({ ...event.data.result, loading: false, chapterId, original, corrected, cached: false });
      } else {
        setState({ ...emptyDiffState(chapterId, original, corrected), error: event.data.error || "差异计算失败" });
      }
    };
    worker.onerror = () => {
      if (!cancelled && requestIdRef.current === requestId) {
        setState({ ...emptyDiffState(chapterId, original, corrected), error: "差异计算失败" });
      }
    };
    worker.postMessage({ requestId, original, rewrite: corrected });
    return () => {
      cancelled = true;
      worker.terminate();
    };
  }, [chapterId, enabled, original, corrected]);

  if (state.chapterId === chapterId && state.original === original && state.corrected === corrected) return state;
  return emptyDiffState(chapterId, original, corrected, enabled && Boolean(corrected.trim()));
}
...
export const CompareView = memo(function CompareView(props: CompareViewProps) {
  const {
    chapters, selectedChapterId, busy, originalRef, correctedRef,
  ...
  const correctedText = selectedChapter?.corrected_text ?? "";
  const diff = useChapterDiff(selectedChapterId, originalText, correctedText, diffEnabled);
  ...
  const correctedMatches = useMemo(() => chapterMatches.filter((match) => match.side === "corrected"), [chapterMatches]);
  const editDirty = editing && editDraft !== correctedText;
  const hasCorrected = Boolean(selectedChapter?.corrected_text?.trim());
  ...
  async function saveEdit(closeAfterSave = true) {
    ...
    setEditBusy(true);
    try {
      await onSaveCorrected(selectedChapter.id, editDraft);
  ...
  useEffect(() => {
    setEditing(false);
    setEditDraft(selectedChapter?.corrected_text ?? "");
    pendingNavigationRef.current = null;
    setPendingNavigation(false);
  }, [selectedChapterId]);
      ...
      <div className="compare-page-toolbar">
        ...
        <div className="compare-toolbar-row compare-toolbar-secondary-row">
          ...
          {hasCorrected && (
            <button
              onClick={() => {
                if (!window.confirm("恢复到审查前的原文？当前审查结果将被覆盖。")) return;
                setEditBusy(true);
      ...
      {searchOpen && (
        <div className="compare-search-bar" role="search">
          ...
          <input
            ref={searchInputRef}
            aria-label="全局搜索"
            placeholder={searchScope === "both" ? "搜索全部章节的原文和审查结果" : searchScope === "original" ? "仅搜索全部章节的原文" : "仅搜索全部章节的审查结果"}
            value={query}
          ...
          >
            <option value="both">原文和审查结果</option>
            <option value="original">仅原文</option>
            <option value="corrected">仅审查结果</option>
          </select>
      ...
      {selectedChapter ? (
        <div className="large-compare-grid">
          ...
          <TextPane
            ...
            text={correctedText}
            emptyText="尚未审查。"
            containerRef={correctedRef}
            diffRanges={visibleDiffRanges}
            searchMatches={correctedMatches}
            activeMatchId={activeMatch?.id}
            headerActions={(
      ...
      {pendingNavigation && (
        <Modal className="settings-dialog compare-unsaved-dialog" labelledBy="compare-unsaved-title">
          <header className="dialog-titlebar">
            <h2 id="compare-unsaved-title">审查结果尚未保存</h2>
          </header>
